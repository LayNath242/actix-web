...
pub fn create(_user: LoggedUser, new_role: web::Json<NewRole>, pool: web::Data<PgPool>) ->
 Result<HttpResponse> {
    let pg_pool = pg_pool_handler(pool)?;
    new_role
        .create(&pg_pool)
...
pub fn update(_user: LoggedUser, id: web::Path<i32>,  new_role: web::Json<NewRole>, pool: web::Data<PgPool>) -> Result<HttpResponse> {
    let pg_pool = pg_pool_handler(pool)?;
